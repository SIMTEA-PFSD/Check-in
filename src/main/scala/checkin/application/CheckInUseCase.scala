 ...
 *
 * Recibe los puertos (traits) por constructor — NO conoce a Kafka ni
 * PostgreSQL. Esto hace al caso de uso trivial de testear con mocks.
 *
 * Usa for-comprehension sobre Either para corto-circuitar en el primer
 * error, sin if/else anidados. Es el equivalente funcional a una
 * cadena de pasos que puede fallar.
 */
class CheckInUseCase(
  ...
  /** Peso máximo permitido por maleta (política del negocio). */
  ...
  private def registrarEquipajes(
    cmd:      CheckInCommand,
    pasajero: Pasajero
  ): Either[CheckInError, List[Equipaje]] = {
  ...
  private def publicarEventos(
    equipajes: List[Equipaje],
    vueloId:   String
  ): Either[CheckInError, Unit] = {
    val eventos = equipajes.map { e =>
      EventoEquipaje.pasajeroRegistrado(e.id, e.pasajeroId, vueloId)
    }
